...
export default async function BlogPostPage({
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      {post.category?.name && (
        <span className="mt-4 inline-block rounded-full bg-accent-gold/10 px-3 py-1 text-xs font-medium uppercase tracking-wide text-accent-gold-dark">
          {post.category.name}
        </span>
      )}
      ...
      {post.bodyHtml && (
        <div
          className="mt-8 text-[15px] leading-relaxed text-text-primary-dark [&_a]:text-accent-gold [&_a]:underline [&_blockquote]:border-l-2 [&_blockquote]:border-accent-gold [&_blockquote]:pl-4 [&_blockquote]:italic [&_blockquote]:text-text-muted [&_em]:italic [&_h2]:mt-8 [&_h2]:font-heading [&_h2]:text-2xl [&_h2]:text-text-primary-dark [&_h3]:mt-6 [&_h3]:font-heading [&_h3]:text-xl [&_h3]:text-text-primary-dark [&_h4]:mt-4 [&_h4]:font-heading [&_h4]:text-lg [&_h4]:text-text-primary-dark [&_li]:mt-1 [&_ol]:mt-4 [&_ol]:list-decimal [&_ol]:pl-6 [&_p]:mt-4 [&_p:first-child]:mt-0 [&_strong]:font-semibold [&_ul]:mt-4 [&_ul]:list-disc [&_ul]:pl-6"
